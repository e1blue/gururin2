...
static void draw(struct pageCartridgeStory *this){
	...
	if(this->isStart){
		...
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_EQ0);
...
//			}
		...
		// ハコニワに隠れない部分の描画
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_GE1_WRITE_INCR);
		for(int i = 0; i < this->stat->charaLength; i++){drawChara(this, i, &worldMat);}
		// ハコニワに隠れる部分のステンシル描画
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_EQ1_MASK_0);
		engineGraphicEngineIgnoreDepthMode(true);
		for(int i = 0; i < this->stat->charaLength; i++){drawChara(this, i, &worldMat);}
