...
static void drawRing(struct pageCartridgeRoll *this){
	...
	engineGraphicEngineSetColorRgba(1.0, 1.0, 1.0, 0.5);
...
static void draw(struct pageCartridgeRoll *this){
	...
	if(isStart){
		...
		// 背景マスク作成
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_EQ1_MASK_2);
		e3dImageDraw(&map->imgBack);
		// 壁描画
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_EQ1_WRITE_0);
		...
		engineGraphicEngineSetStencilMode(ENGINEGRAPHICENGINEMODESTENCIL_READ_EQ2);
